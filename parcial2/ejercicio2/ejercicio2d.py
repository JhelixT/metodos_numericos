...
def main():
    ...
    X, fp = diferenciacion(f, 0, 1, 10, verbose=False)

    print("Tabla de valores Derivada de f(x) de 0 a 1")
    print("="*90)
    print(f"Xi = {X}")
    print(f"f'(Xi) = {fp}")

    integral = trapecio(X=X, Y=fp, verbose=False)
    print(f"La integral con datos tabulados de f'(x) es: {integral}")
    print(f"El error absoluto exacto es {abs(integral-(math.sqrt(2)-1))}")
...
def diferenciacion(f, a, b, n, verbose=True):
    ...
    # Fórmula de 2 puntos hacia adelante para el punto inicial (precisión O(h))
    fp[0] = (f(X[1])-f(X[0])) / (h)
    ...
    # Fórmula de 2 puntos hacia atrás para el punto final (precisión O(h))
    fp[n] = (f(X[n])-f(X[n-1])) / (h)
