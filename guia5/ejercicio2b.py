...
def main():
    valores_x = [0, 0.5,-1,2]
    archivo = os.path.join(os.path.dirname(__file__), "inputs", "datos2b.txt")
    generar_datos(valores_x, archivo, separador=" ")
    p, coef, X, Y = f.interpolacion(nombre_archivo=archivo)
    f.graficar_interpolacion(p, coef, X, Y, funcion_real=lambda x: 4*x**3-3*x**2+2)
...
def comparar_error(P, xmin, xmax, puntos=500):
    ...
    plt.subplot(1,2,1)
    plt.plot(x_dense, y_real, "g--", label="f(x) = 4x^3-3x^2+2")
