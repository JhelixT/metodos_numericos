...
def main():
    valores_x = [0, 0.5, 1.2, 0.3, -1, 0.9, -1.5]
    archivo = os.path.join(os.path.dirname(__file__), "inputs", "datos2a.txt")
    generar_datos(valores_x, archivo, separador=" ")
    p, coef, X, Y = f.interpolacion(nombre_archivo=archivo)
    # f.graficar_interpolacion(p, coef, X, Y, funcion_real=lambda x: np.exp(-x**2))
    print("Coeficientes del polinomio (ordenados por potencia):")
    for i, c in enumerate(coef):
        print(f"x^{i}: {c}")
    comparar_error(p, max(X), max(X)+2)
...
def comparar_error(P, xmin, xmax, puntos=500):
    ...
    plt.subplot(1,2,1)
    plt.plot(x_dense, y_real, "g--", label="f(x) = e^(-x^2)")
